...
/*
 * queue_iterate - Call a user-supplied function on all items of a queue
 ...
 *
 * In the case where @func returns 1 for a specific item, then the iteration
 * actually stops and queue_iterate should return early. Before returning, if
 * @data is different than NULL, then it receives the data item where the
 ...
 * successfully (even if the iteration was stopped early because @func returned
 * 1 for a specific item).
 */
